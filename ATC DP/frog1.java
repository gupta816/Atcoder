...
public void close() throws IOException 
    ...
    public static int[] prefix=new int[MAX+1];
    ...
    public static int frog(int id,int a[],int n)
    {
    	if(id==n-1)
    	{
    		return 0;
    	...
    	int ans=frog(id+1,a,n)+Math.abs(a[id+1]-a[id]);
    	if(id+2<n)
    	{
    		ans=Math.min(ans, frog(id+2,a,n)+Math.abs(a[id+2]-a[id]));
    	}
    	return dp[id]=ans;
